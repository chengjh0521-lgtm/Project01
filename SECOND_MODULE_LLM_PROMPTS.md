...
本文档按当前代码实际调用路径整理第二模块的五类 DeepSeek 提示词，供修改、评审和回归测试使用。最后核对：`2026-07-20`。

处理顺序为：`字幕1（ASR）` -> `字幕2（洗稿）` -> `高光时间段/问题/理由` -> `高光字幕3` -> `关键词` -> `音效决策` 和 `视觉素材决策`。音效与视觉素材均基于同一份高光字幕和关键词独立决策。

| 阶段 | 实际文件 | 调用次数 |
| --- | --- | --- |
| 1. 字幕洗稿 | `subtitle_processing/pipeline.py` | 按 30 条字幕一批，多批调用 |
| 2. 高光剪辑 | `subtitle_processing/multi_highlight_stage.py` | 每条候选最多重试 3 次 |
| 3. 高光关键词 | `subtitle_processing/keyword_stage.py` | 每条高光 1 次 |
| 4. 音效决策 | `subtitle_processing/sound_effect_binding.py` | 每条高光 1 次 |
| 5. GIF/PNG 素材决策 | `subtitle_processing/visual_asset_binding.py` | 每条高光 1 次；覆盖不足时额外重试 1 次 |

除非明确写为“动态追加的用户提示词”，以下代码块均为实际传入模型的 system prompt。模型、API Key、超时、重试等传输参数不在本文档范围内。
...
{包含 target_entries、target_ids、context_entries 的 JSON}
```

说明：`CORRECTION_USER_PROMPT` 这个中文常量仍保留在源码中，但当前批处理调用并未使用它；上面的英文文本才是线上实际请求内容。
...
已选素材如下：
{此前已选素材的问题与时间段；第一次为“无”}

请提取一个新的、能完整回答明确问题的主题或不同角度。新问题不能与已选问题重复；新素材与所有已选素材的重合时长不得超过新素材总时长的 30%。如果无法提取合规的新素材，返回 {"question":"","ranges":[],"reason":""}。

This is attempt {1-3} of 3. Return the exact JSON object only.
```

后端会校验：问题非空且不超过 18 个字符、时间戳有效、与已选素材的重合不超过 30%。任一条件不满足时会对同一候选重试，最多 3 次。
...
后端只保留确实出现在高光 SRT 中的关键词，并最多按网页输入的预期数量输出。

## 4. 音效添加

来源：`subtitle_processing/sound_effect_binding.py` 中的 `SOUND_CUE_SYSTEM_PROMPT`。

注：文件中另有 `SOUND_BINDING_SYSTEM_PROMPT`，它是旧的“逐关键词绑定”兼容提示词；当前第二模块实际运行的是下面的逐句 `SOUND_CUE_SYSTEM_PROMPT`。
    ...
    "sound_effects": [
      {
        ...
        "example_keywords": [],
        "strength": "...",
        "recommended_max_per_30s": 1,
        "avoid_scenes": []
      }
    ]
  },
  "sentences": [
    {"sentence_id": 1, "text": "...", "keywords": ["..."]}
  ]
}
```

只有“音效文件存在且该音效已有描述”的音效会被传给模型并允许选择。实际用户提示词为：
...
实际输入中的 `asset_index.assets` 是运行时合并后的可用素材：系统会读取 `visual_assets` 目录中所有兼容的 JSON 索引文件，跳过缺失的素材文件和重复的素材 ID。实际用户提示词为：

```text
请严格按系统要求逐句选择视觉素材，只返回 JSON。
```

当首轮结果未达到“至少 2 个素材且总时长严格大于 5 秒”的全局约束时，系统会把相同输入再调用一次，并追加：
