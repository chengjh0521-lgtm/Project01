...
class FourPartHighlightRenderTests(unittest.TestCase):
    def test_renders_intro_doctor_answer_then_main_highlight(self):
        clip = {
            "id": "clip_01",
            "question": "糖尿病能喝酒吗？",
        ...
        with patch("video_generation.four_part.render_highlight_video", side_effect=render_results) as render, patch(
            "video_generation.four_part.create_question_intro", return_value="/tmp/intro.mp4"
        ) as intro, patch(
        ...
        self.assertIn("three-part sequence=True", message)
        ...
        concat.assert_called_once_with(
            ["/tmp/intro.mp4", "/tmp/answer.mp4", "/tmp/main.mp4"],
    ...
    def test_skips_answer_section_when_it_is_already_in_the_main_opening(self):
        ...
        with patch("video_generation.four_part.render_highlight_video", return_value=("/tmp/main.mp4", None, "main", "main srt")) as render, patch(
            "video_generation.four_part.create_question_intro", return_value="/tmp/intro.mp4"), patch(
            "video_generation.four_part.concat_video_segments", return_value="/tmp/final.mp4"
        ) as concat:
        ...
        self.assertEqual(concat.call_args.args[0], ["/tmp/intro.mp4", "/tmp/main.mp4"])
