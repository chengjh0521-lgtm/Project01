...
if __name__ == "__main__":
    ...
    def AI_clip_subti(LLM_res, video_state, audio_state, subtitle_srt, output_dir, font_size, font_color, subtitle_x, subtitle_y, highlight_terms, highlight_color, sound_effect_rules, selected_sfx, selected_sfx_terms):
        ...
        logging.warning(
            "Final clip rendering uses corrected step-2 SRT %s exclusively.",
        ...
        if video_state is not None:
            ...
            message = (
                _llm_clip_range_message(timestamp_list)
                + f"\nCorrected step-2 subtitle SRT: {subtitle_fingerprint}"
                + "\n"
                + message
            )
            logging.warning(message)
            ...
            with gr.Column():
                ...
                with gr.Accordion("DeepSeek 字幕修正 | Subtitle Correction", open=True):
                    subtitle_correction_prompt = gr.Textbox(
                        ...
                        value=(
                            DEFAULT_SUBTITLE_CORRECTION_PROMPT
                            if user_settings.get("subtitle_correction_prompt")
                            in (None, "", LEGACY_DEFAULT_SUBTITLE_CORRECTION_PROMPT)
                            else user_settings.get("subtitle_correction_prompt")
                        ),
                        lines=5,
                    )
