...
if __name__ == "__main__":
    ...
    def _run_asr_task(job_id, speaker_mode, local_video, video_input, audio_input, hotwords, output_dir):
        ...
        try:
            with ASR_RUN_LOCK:
                ...
                if speaker_mode:
                    result = mix_recog_speaker(local_video, video_input, audio_input, hotwords, output_dir)
                else:
                    result = mix_recog(local_video, video_input, audio_input, hotwords, output_dir)
            if result is None:
                raise ValueError("No local video, uploaded video, or audio input was provided.")
            _set_asr_task(
                job_id,
                status="done",
                message="ASR completed. Results are loaded below.",
                result=result,
    ...
    def _start_asr_task(local_video, video_input, audio_input, hotwords, output_dir, speaker_mode=False):
        if not local_video and video_input is None and audio_input is None:
            return "Please choose a server local video, upload a video, or upload audio first.", "", "", None, None, None, None, "", ""

        job_id = datetime.now().strftime("%Y%m%d_%H%M%S_%f")
        video_input = _copy_video_input_for_background(video_input, job_id)
        _set_asr_task(
            job_id,
            status="starting",
        ...
        thread = threading.Thread(
            ...
            args=(job_id, speaker_mode, local_video, video_input, audio_input, hotwords, output_dir),
            daemon=True,
        )
        thread.start()
        mode_name = "ASR+SD" if speaker_mode else "ASR"
        return (
            f"{mode_name} task started. Job ID: {job_id}\nYou can keep this page open and click Refresh ASR Task, or come back later while the backend continues running.",
            "",
            "",
            None,
            None,
    ...
    def check_asr_task(job_id):
        if not job_id:
            return gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), job_id, job_id

        task = _get_asr_task(job_id)
        if not task:
            return f"ASR task not found: {job_id}", gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), job_id, job_id

        status = task.get("status", "unknown")
        message = task.get("message", "")
        if status == "done":
            res_text, res_srt, video_state, audio_state, text_file, srt_file = task.get("result")
            return f"Done. Job ID: {job_id}\n{message}", res_text, res_srt, video_state, audio_state, text_file, srt_file, job_id, job_id
        if status == "failed":
            detail = task.get("traceback") or ""
            return f"Failed. Job ID: {job_id}\n{message}\n{detail[-2000:]}", gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), job_id, job_id
        return f"{status.title()}. Job ID: {job_id}\n{message}", gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), gr.update(), job_id, job_id
    ...
    with gr.Blocks(theme=theme, css=app_css) as funclip_service:
                ...
                with gr.Column():
                    ...
                    with gr.Column():
                        ...
                        with gr.Row():
                            ...
                            refresh_asr_task_button = gr.Button("Refresh ASR Task")
                        asr_task_id_input = gr.Textbox(label="ASR Job ID", interactive=True)
        ...
        recog_button.click(start_asr_task,
                            ...
                            outputs=[asr_task_status, video_text_output, video_srt_output, video_state, audio_state, video_text_file, video_srt_file, asr_task_state, asr_task_id_input])
        recog_button2.click(start_asr_speaker_task,
                            ...
                            outputs=[asr_task_status, video_text_output, video_srt_output, video_state, audio_state, video_text_file, video_srt_file, asr_task_state, asr_task_id_input])
        refresh_asr_task_button.click(
                            check_asr_task,
                            inputs=[asr_task_id_input],
                            outputs=[asr_task_status, video_text_output, video_srt_output, video_state, audio_state, video_text_file, video_srt_file, asr_task_state, asr_task_id_input])
        asr_task_timer.tick(
                            check_asr_task,
                            inputs=[asr_task_id_input],
